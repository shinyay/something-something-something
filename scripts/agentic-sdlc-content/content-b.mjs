...
${callout(
    ...
    したがってこの段は<strong>既存の可観測性基盤 ＋ 人間の運用プロセス</strong>で実装するしかありません。Copilot 側で唯一この方向にあるのが ${PP} <strong>Copilot Memory</strong>（リポジトリ事実の永続化・28 日間未使用で失効）ですが、これは運用シグナルの取り込み機構ではありません。提案時に「ここは製品機能ではなく運用設計」と明示することが誠実です。</p>`,
)}
