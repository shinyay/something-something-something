// PART 6: sections 13 (glossary), 14 (sources), 15 (deck-map appendix).
...
// Slide-number chips. Used only in the deck-map appendix (§15).
const chips = (...nums) =>
...
${table(
    ...
    [
        ...
        [
            ...
            FRAMEWORK,
            "公式のアーキテクチャ表現ではない。理解のための整理層",
        ],
        [
            "<strong>Agentic Modernization Loop</strong>（5 変換）",
            FRAMEWORK,
            `部分的に Modernize CLI の Assess → Plan → Execute に対応する（§06）`,
        ],
        [
            "<strong>Evidence Package</strong>（Code + Evidence + Uncertainty）",
            FRAMEWORK,
            "session log ＋ checks 結果 ＋ Verified 署名コミットが素材。<strong>受け入れ条件へのマッピングは自動化されない</strong>（§08）",
        ],
        [
...
${ul([
    a("https://docs.github.com/en/copilot/reference/supported-surfaces-for-policies", "Supported surfaces for policies") + `<span class="src-note">§09 の content exclusion 対応表の出典。<strong>「cloud agent には効かない」という誤解を正す根拠</strong></span>`,
...
${table(
    ...
    [
        ...
        [`<strong>§06</strong> モダナイゼーション専用ツール`, chips(28, 29, 30)],
        [`<strong>§07</strong> 確率的推論・決定的検証`, chips(24, 28, 35)],
        [`<strong>§08</strong> ガバナンス境界としての PR`, chips(32, 33, 35, 36, 37)],
        [`<strong>§09</strong> セキュリティ`, chips(38)],
        [`<strong>§10</strong> 採用ラダー`, chips(42, 43, 44)],
        [`<strong>§11</strong> コスト構造`, `<span class="muted">登壇資料は課金に触れていない（本資料で補った論点）</span>`],
        [`<strong>§12</strong> 委譲してはいけない作業`, `<span class="muted">本資料で新規に加えた実践論点</span>`],
        [`<strong>§13</strong> 用語集`, chips(8, 12, 13, 20, 22, 23, 25, 35, 43)],
        [`<strong>§14</strong> 一次情報リンク集`, `<span class="muted">一次情報のみ（スライド非依存）</span>`],
        [`<strong>§15</strong> 本付録`, `<span class="muted">—</span>`],
    ],
    { widths: ["55%", "45%"] },
)}
...
${callout(
    "note",
    "スライドは補助であって前提ではない",
    `<p>本資料は §00〜§14 だけで、GitHub Copilot の技術的観点から Agentic SDLC を通読できるように書かれています。上の対応表とダウンロードできる PDF は、登壇の現場で本資料とスライドを併用したい場合の橋渡しにすぎません。<strong>スライドを持っていなくても、本文の理解に不足は生じません。</strong></p>`,
)}
`,
    },
];
